...
public class Main {
	...
	private static Map<String, Integer> compareCatagory(List<Student> students, List<Student> temp, int counter,
		...
		while (low <= mid && high <= right) {
			if (flag == 1) {
				if (students.get(low).getAverageScore() > students.get(high).getAverageScore()) {
					temp.set(counter++, students.get(low));
					low++;
				} else {
					temp.set(counter++, students.get(high));
				...
				if (students.get(low).getCLanguageScores() > students.get(high).getCLanguageScores()) {
					temp.set(counter++, students.get(low));
					low++;
				} else {
					temp.set(counter++, students.get(high));
				...
				if (students.get(low).getMathematicsScores() > students.get(high).getMathematicsScores()) {
					temp.set(counter++, students.get(low));
					low++;
				} else {
					temp.set(counter++, students.get(high));
				...
				if (students.get(low).getEnglishScores() > students.get(high).getEnglishScores()) {
					temp.set(counter++, students.get(low));
					low++;
				} else {
					temp.set(counter++, students.get(high));
